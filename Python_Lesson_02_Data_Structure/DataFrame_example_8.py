...
# mean 方法統計平均值
# 默认情况下，缺失的值不会参与sum和mean的计算，这和 Excel 对待空单元格的方式是一样的
# 默認統計列的平均值
print(numbers.mean())
...
# groupby 方法分組數據，結合 agg 方法使用自定義函數
print(df.loc[:, ['No.', 'score', 'job']].groupby(['job']).agg(lambda x: x.max() - x.min()))

# pivot_table 函数的功能類似於Excel的数据透视表
...
# melt 函數将列标题转换成列的值，如果想将数据处理成数据库要求的格式，可以使用melt
